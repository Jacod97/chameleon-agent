...
scripts/train.py 는 처음부터 학습하는 순수 조립 스크립트로 그대로 두고,
저장된 체크포인트에서 이어 학습하는 흐름은 이 파일이 전담한다.

사용:
    python scripts/resume_train.py resume_path=results/run_01/model_400.pt start_stage=5 save_dir=results/run_02
"""
import sys
...
def main(cfg: DictConfig):
    # fail-fast: 이 진입점은 재개 전용 — 처음부터 학습하려면 scripts/train.py 사용
    if not cfg.get("resume_path"):
        raise ValueError("resume_path 필요 — 예: resume_path=results/run_01/model_400.pt "
                         "(처음부터 학습은 scripts/train.py)")
    ...
    logger.log_params({
        ...
        "resume_path": cfg["resume_path"], "start_stage": cfg.get("start_stage", 1),
    })
