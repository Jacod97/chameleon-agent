...
class ActorCritic(nn.Module):
    ...
    def get_action(self, observation_vector: torch.Tensor, point_cloud: torch.Tensor):
        ...
        return continuous_action, discrete_action_tensor, log_prob, value
    ...
    def evaluate(
        ...
        continuous_clamped = continuous_action.clamp(-1.0 + 1e-6, 1.0 - 1e-6)
        u = torch.atanh(continuous_clamped)
        log_prob = continuous_dist.log_prob(u).sum(-1) - torch.log(1.0 - continuous_clamped.pow(2) + 1e-6).sum(-1)
        entropy = continuous_dist.entropy().sum(-1)
